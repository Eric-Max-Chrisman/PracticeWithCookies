...
import { addBook, getBookById, getAllBooks } from '../models/bookModel';
import { parseDatabaseError } from '../utils/db-utils';

async function getAllBooksCon(req: Request, res: Response): Promise<void> {
...
async function registerBook(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const newUser = await addBook(title, publicationYear);
    console.log(newUser);
    res.sendStatus(201);
  } catch (err) {
    console.error(err);
